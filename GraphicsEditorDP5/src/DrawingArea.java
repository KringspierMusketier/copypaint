...
public class DrawingArea extends JComponent {
		...
		addMouseListener(new MouseAdapter( ) {
			...
			public void mousePressed(MouseEvent e) {
				if (r.getTool() == Tool.DRAW) {
					...
					ShapeLeaf s = new ShapeLeaf(oldX, oldY, (currentX - oldX), (currentY - oldY), r.getShapeType().getStrategy(), r.getSelectedColor());
	...
	protected void paintComponent(Graphics g) {
		...
		r.getExpandRect().draw(g2);
		r.getSelectRect().draw(g2);
		...
		if (r.getShapes().size() > 0) {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				try (FileWriter fw = new FileWriter(fc.getSelectedFile() + ".txt")) {
					for (Shape s : r.getShapes()) {
						for (String str : s.accept(new ShapePrintVisitor())) {
							fw.write(str);
						}
					}
				} catch (IOException e) {
			...
			try (FileReader fr = new FileReader(file)) {
				...
				for (int z = 0; z < lines.size(); z++) {
					...
					if (src[0].contentEquals("group")) {
						...
						ShapeLeaf shape = new ShapeLeaf(Integer.parseInt(src[2]), Integer.parseInt(src[3]), Integer.parseInt(src[4]), Integer.parseInt(src[5]), ShapeType.valueOf(src[0].toUpperCase()).getStrategy(), NamedColor.valueOf(src[1].toUpperCase()));
